...
class ML_Predictor:
    ...
    def compile_all_circuits_for_qc(
        ...
        if source_path is None:
            source_path = str(ML_utils.get_path_training_circuits_ML())

        if target_path is None:
            target_path = str(ML_utils.get_path_training_circuits_ML_compiled())
    ...
    def generate_training_sample(
        ...
        for i in range(len(LUT)):
            if not scores[i]:
                scores[i] = ML_utils.get_width_penalty()
            else:
        ...
        feature_vec = ML_utils.create_feature_dict(str(Path(source_path) / file))
    ...
    def get_prepared_training_data(self, save_non_zero_indices=False):
        ...
        if save_non_zero_indices:
            ...
            np.save(ML_utils.get_path_trained_model_ML() / "non_zero_indices.npy", data)
    ...
    def plot_eval_histogram(
        ...
        num_of_comp_paths = len(ML_utils.get_index_to_comppath_LUT())
        plt.bar(
    ...
    def predict(self, qasm_str_or_path: str):
        ...
        if self.clf is None:
            path = ML_utils.get_path_trained_model_ML() / "trained_clf.joblib"
            if path.is_file():
        ...
        feature_dict = ML_utils.create_feature_dict(qasm_str_or_path)
        if not feature_dict:
            return None
        feature_vector = list(feature_dict.values())

        path = ML_utils.get_path_trained_model_ML() / "non_zero_indices.npy"
