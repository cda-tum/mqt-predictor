OPENQASM 2.0;
include "qelib1.inc";

qreg node[27];
creg meas[15];
sx node[10];
sx node[12];
sx node[14];
sx node[15];
sx node[16];
sx node[17];
sx node[18];
sx node[19];
sx node[20];
sx node[21];
sx node[22];
sx node[23];
sx node[24];
sx node[25];
sx node[26];
rz(0.5*pi) node[10];
rz(0.5*pi) node[12];
rz(0.5*pi) node[14];
rz(0.5*pi) node[15];
rz(0.5*pi) node[16];
rz(0.5*pi) node[17];
rz(0.5*pi) node[18];
rz(0.5*pi) node[19];
rz(0.5*pi) node[20];
rz(0.5*pi) node[21];
rz(0.5*pi) node[22];
rz(0.5*pi) node[23];
rz(0.5*pi) node[24];
rz(0.5*pi) node[25];
rz(0.5*pi) node[26];
sx node[10];
sx node[12];
sx node[14];
sx node[15];
sx node[16];
sx node[17];
sx node[18];
sx node[19];
sx node[20];
sx node[21];
sx node[22];
sx node[23];
sx node[24];
sx node[25];
sx node[26];
rz(0.5*pi) node[14];
rz(0.5*pi) node[15];
cx node[16],node[19];
rz(0.5*pi) node[17];
cx node[18],node[21];
rz(0.5*pi) node[20];
cx node[22],node[25];
cx node[23],node[24];
rz(0.5*pi) node[26];
rz(1.0806471402905593*pi) node[19];
rz(1.0806471402905593*pi) node[21];
rz(1.0806471402905593*pi) node[24];
rz(1.0806471402905593*pi) node[25];
cx node[16],node[19];
cx node[18],node[21];
cx node[22],node[25];
cx node[23],node[24];
cx node[17],node[18];
cx node[20],node[19];
cx node[25],node[22];
cx node[18],node[17];
rz(1.0806471402905593*pi) node[19];
cx node[22],node[25];
cx node[17],node[18];
cx node[20],node[19];
cx node[25],node[22];
cx node[18],node[21];
rz(0.5*pi) node[19];
cx node[26],node[25];
cx node[21],node[18];
sx node[19];
rz(1.0806471402905593*pi) node[25];
cx node[18],node[21];
rz(1.5638330033927854*pi) node[19];
cx node[26],node[25];
cx node[15],node[18];
sx node[19];
cx node[23],node[21];
rz(0.5*pi) node[25];
cx node[18],node[15];
rz(0.5*pi) node[19];
cx node[21],node[23];
sx node[25];
cx node[15],node[18];
cx node[16],node[19];
cx node[23],node[21];
rz(1.5638330033927854*pi) node[25];
cx node[19],node[16];
cx node[23],node[24];
sx node[25];
cx node[16],node[19];
rz(1.0806471402905593*pi) node[24];
rz(0.5*pi) node[25];
cx node[22],node[19];
cx node[23],node[24];
cx node[19],node[22];
cx node[23],node[21];
rz(0.5*pi) node[24];
cx node[22],node[19];
cx node[21],node[23];
sx node[24];
cx node[19],node[16];
cx node[23],node[21];
cx node[25],node[22];
rz(1.5638330033927854*pi) node[24];
cx node[16],node[19];
cx node[21],node[18];
cx node[22],node[25];
sx node[24];
cx node[19],node[16];
cx node[18],node[21];
cx node[25],node[22];
rz(0.5*pi) node[24];
cx node[21],node[18];
cx node[22],node[19];
cx node[26],node[25];
cx node[18],node[15];
cx node[19],node[22];
cx node[21],node[23];
rz(1.0806471402905593*pi) node[25];
cx node[15],node[18];
cx node[22],node[19];
rz(1.0806471402905593*pi) node[23];
cx node[26],node[25];
cx node[18],node[15];
cx node[21],node[23];
rz(0.5*pi) node[25];
sx node[26];
cx node[15],node[12];
cx node[21],node[18];
rz(0.5*pi) node[23];
sx node[25];
rz(1.5638330033927854*pi) node[26];
rz(1.0806471402905593*pi) node[12];
rz(1.0806471402905593*pi) node[18];
sx node[23];
rz(1.5638330033927854*pi) node[25];
sx node[26];
cx node[15],node[12];
cx node[21],node[18];
rz(1.5638330033927854*pi) node[23];
sx node[25];
rz(1.0*pi) node[26];
cx node[10],node[12];
sx node[15];
rz(0.5*pi) node[18];
sx node[21];
sx node[23];
rz(0.5*pi) node[25];
rz(1.0806471402905593*pi) node[12];
rz(1.5638330033927854*pi) node[15];
sx node[18];
rz(1.5638330033927854*pi) node[21];
cx node[25],node[22];
rz(0.5*pi) node[23];
cx node[10],node[12];
sx node[15];
rz(1.5638330033927854*pi) node[18];
sx node[21];
rz(1.5276028477869952*pi) node[22];
cx node[23],node[24];
rz(0.5*pi) node[12];
rz(1.0*pi) node[15];
sx node[18];
rz(1.0*pi) node[21];
cx node[25],node[22];
rz(1.5276028477869952*pi) node[24];
sx node[12];
rz(0.5*pi) node[18];
cx node[23],node[24];
cx node[26],node[25];
rz(1.5638330033927854*pi) node[12];
cx node[17],node[18];
cx node[21],node[23];
cx node[25],node[26];
sx node[12];
cx node[18],node[17];
rz(1.5276028477869952*pi) node[23];
cx node[26],node[25];
rz(0.5*pi) node[12];
cx node[17],node[18];
cx node[21],node[23];
cx node[10],node[12];
cx node[18],node[15];
rz(0.5*pi) node[23];
cx node[12],node[10];
cx node[15],node[18];
sx node[23];
cx node[10],node[12];
cx node[18],node[15];
rz(2.2961325361261484*pi) node[23];
cx node[12],node[13];
sx node[23];
cx node[13],node[12];
rz(0.5*pi) node[23];
cx node[12],node[13];
cx node[24],node[23];
cx node[10],node[12];
cx node[13],node[14];
cx node[23],node[24];
cx node[12],node[10];
cx node[14],node[13];
cx node[24],node[23];
cx node[10],node[12];
cx node[13],node[14];
cx node[23],node[21];
cx node[12],node[15];
cx node[16],node[14];
cx node[21],node[23];
cx node[15],node[12];
cx node[14],node[16];
cx node[23],node[21];
cx node[12],node[15];
cx node[16],node[14];
cx node[18],node[21];
cx node[13],node[14];
cx node[19],node[16];
rz(1.5276028477869952*pi) node[21];
rz(1.0806471402905593*pi) node[14];
cx node[16],node[19];
cx node[18],node[21];
cx node[13],node[14];
cx node[18],node[15];
cx node[19],node[16];
rz(0.5*pi) node[21];
cx node[13],node[12];
rz(0.5*pi) node[14];
rz(1.5276028477869952*pi) node[15];
cx node[20],node[19];
sx node[21];
rz(1.0806471402905593*pi) node[12];
sx node[14];
cx node[18],node[15];
rz(1.0806471402905593*pi) node[19];
rz(2.2961325361261484*pi) node[21];
cx node[13],node[12];
rz(1.5638330033927854*pi) node[14];
sx node[18];
cx node[20],node[19];
sx node[21];
rz(0.5*pi) node[12];
sx node[13];
sx node[14];
rz(2.2961325361261484*pi) node[18];
rz(0.5*pi) node[19];
sx node[20];
rz(0.5*pi) node[21];
sx node[12];
rz(1.5638330033927854*pi) node[13];
rz(0.5*pi) node[14];
sx node[18];
sx node[19];
rz(1.5638330033927854*pi) node[20];
cx node[23],node[21];
rz(1.5638330033927854*pi) node[12];
sx node[13];
cx node[16],node[14];
rz(0.5*pi) node[18];
rz(1.5638330033927854*pi) node[19];
sx node[20];
cx node[21],node[23];
sx node[12];
rz(1.0*pi) node[13];
rz(1.5276028477869952*pi) node[14];
cx node[17],node[18];
sx node[19];
rz(1.0*pi) node[20];
cx node[23],node[21];
rz(0.5*pi) node[12];
cx node[16],node[14];
cx node[18],node[17];
rz(0.5*pi) node[19];
cx node[12],node[15];
cx node[13],node[14];
cx node[19],node[16];
cx node[17],node[18];
cx node[15],node[12];
rz(1.5276028477869952*pi) node[14];
cx node[16],node[19];
cx node[12],node[15];
cx node[13],node[14];
cx node[19],node[16];
cx node[12],node[13];
rz(0.5*pi) node[14];
cx node[15],node[18];
cx node[19],node[22];
cx node[13],node[12];
sx node[14];
rz(1.5276028477869952*pi) node[18];
cx node[22],node[19];
cx node[12],node[13];
rz(2.2961325361261484*pi) node[14];
cx node[15],node[18];
cx node[19],node[22];
cx node[12],node[15];
sx node[14];
cx node[21],node[18];
cx node[20],node[19];
cx node[25],node[22];
rz(0.5*pi) node[14];
rz(1.5276028477869952*pi) node[15];
rz(1.5276028477869952*pi) node[18];
rz(1.5276028477869952*pi) node[19];
rz(1.5276028477869952*pi) node[22];
cx node[12],node[15];
cx node[13],node[14];
cx node[21],node[18];
cx node[20],node[19];
cx node[25],node[22];
sx node[12];
cx node[14],node[13];
rz(0.5*pi) node[15];
rz(0.5*pi) node[18];
rz(0.5*pi) node[19];
sx node[21];
rz(0.5*pi) node[22];
cx node[25],node[26];
rz(2.2961325361261484*pi) node[12];
cx node[13],node[14];
sx node[15];
sx node[18];
sx node[19];
rz(2.2961325361261484*pi) node[21];
sx node[22];
rz(1.5276028477869952*pi) node[26];
sx node[12];
cx node[16],node[14];
rz(2.2961325361261484*pi) node[15];
rz(2.2961325361261484*pi) node[18];
rz(2.2961325361261484*pi) node[19];
sx node[21];
rz(2.2961325361261484*pi) node[22];
cx node[25],node[26];
rz(0.5*pi) node[12];
rz(1.5276028477869952*pi) node[14];
sx node[15];
sx node[18];
sx node[19];
rz(0.5*pi) node[21];
sx node[22];
sx node[25];
rz(0.5*pi) node[26];
cx node[16],node[14];
rz(0.5*pi) node[15];
rz(0.5*pi) node[18];
rz(0.5*pi) node[19];
rz(0.5*pi) node[22];
rz(2.2961325361261484*pi) node[25];
sx node[26];
rz(0.5*pi) node[14];
cx node[20],node[19];
sx node[25];
rz(2.2961325361261484*pi) node[26];
sx node[14];
cx node[19],node[20];
rz(0.5*pi) node[25];
sx node[26];
rz(2.2961325361261484*pi) node[14];
cx node[20],node[19];
rz(0.5*pi) node[26];
sx node[14];
cx node[19],node[16];
rz(0.5*pi) node[14];
rz(1.5276028477869952*pi) node[16];
cx node[19],node[16];
rz(0.5*pi) node[16];
sx node[19];
sx node[16];
rz(2.2961325361261484*pi) node[19];
rz(2.2961325361261484*pi) node[16];
sx node[19];
sx node[16];
rz(0.5*pi) node[19];
rz(0.5*pi) node[16];
barrier node[25],node[19],node[12],node[26],node[21],node[16],node[17],node[14],node[15],node[20],node[18],node[22],node[24],node[13],node[23];
measure node[25] -> meas[0];
measure node[19] -> meas[1];
measure node[12] -> meas[2];
measure node[26] -> meas[3];
measure node[21] -> meas[4];
measure node[16] -> meas[5];
measure node[17] -> meas[6];
measure node[14] -> meas[7];
measure node[15] -> meas[8];
measure node[20] -> meas[9];
measure node[18] -> meas[10];
measure node[22] -> meas[11];
measure node[24] -> meas[12];
measure node[13] -> meas[13];
measure node[23] -> meas[14];
