OPENQASM 2.0;
include "qelib1.inc";

qreg node[15];
creg meas[6];
sx node[0];
rz(0.5*pi) node[1];
rz(0.5*pi) node[2];
rz(0.5*pi) node[3];
rz(0.5*pi) node[4];
rz(0.5*pi) node[14];
rz(2.022597350686529*pi) node[0];
sx node[1];
sx node[2];
sx node[3];
sx node[4];
sx node[14];
sx node[0];
rz(2.5*pi) node[1];
rz(2.5*pi) node[2];
rz(0.5*pi) node[3];
rz(2.5*pi) node[4];
rz(2.5*pi) node[14];
rz(1.0*pi) node[0];
sx node[1];
sx node[2];
sx node[3];
sx node[4];
sx node[14];
rz(0.33211305264878554*pi) node[1];
rz(0.3912424946536037*pi) node[2];
rz(0.4443488660639947*pi) node[3];
rz(0.3237682959615542*pi) node[4];
rz(0.3661155741202371*pi) node[14];
cx node[0],node[1];
cx node[0],node[14];
sx node[1];
rz(2.5*pi) node[1];
sx node[1];
rz(1.5*pi) node[1];
cx node[0],node[1];
cx node[1],node[0];
cx node[0],node[1];
cx node[0],node[14];
cx node[1],node[2];
cx node[2],node[1];
sx node[14];
cx node[1],node[2];
rz(2.5*pi) node[14];
cx node[2],node[1];
sx node[14];
cx node[0],node[1];
cx node[2],node[3];
rz(1.5*pi) node[14];
cx node[0],node[1];
cx node[3],node[2];
cx node[1],node[0];
cx node[2],node[3];
cx node[0],node[1];
cx node[3],node[2];
cx node[14],node[0];
cx node[1],node[2];
cx node[3],node[4];
sx node[0];
cx node[1],node[2];
sx node[3];
rz(2.5*pi) node[0];
cx node[2],node[1];
rz(3.1793907141516877*pi) node[3];
sx node[0];
cx node[1],node[2];
sx node[3];
rz(1.5*pi) node[0];
rz(1.0*pi) node[3];
cx node[14],node[0];
cx node[4],node[3];
cx node[0],node[14];
cx node[3],node[4];
cx node[14],node[0];
cx node[4],node[3];
cx node[0],node[1];
cx node[2],node[3];
cx node[0],node[1];
rz(0.5*pi) node[2];
cx node[1],node[0];
sx node[2];
cx node[0],node[1];
rz(0.5*pi) node[2];
cx node[14],node[0];
sx node[2];
sx node[0];
rz(3.5438469818097245*pi) node[2];
rz(2.5*pi) node[0];
cx node[2],node[3];
sx node[0];
cx node[3],node[2];
rz(1.5*pi) node[0];
cx node[2],node[3];
cx node[14],node[0];
cx node[1],node[2];
cx node[4],node[3];
cx node[0],node[14];
rz(0.5*pi) node[1];
sx node[3];
cx node[14],node[0];
sx node[1];
rz(2.5*pi) node[3];
rz(0.5*pi) node[1];
sx node[3];
sx node[1];
rz(1.5*pi) node[3];
rz(3.73218416173482*pi) node[1];
cx node[4],node[3];
cx node[1],node[2];
cx node[3],node[4];
cx node[2],node[1];
cx node[4],node[3];
cx node[1],node[2];
cx node[0],node[1];
cx node[3],node[2];
rz(0.5*pi) node[0];
cx node[3],node[2];
sx node[0];
cx node[2],node[3];
rz(0.5*pi) node[0];
cx node[3],node[2];
sx node[0];
cx node[4],node[3];
rz(0.49302573289435514*pi) node[0];
sx node[3];
cx node[1],node[0];
rz(2.5*pi) node[3];
cx node[0],node[1];
sx node[3];
cx node[1],node[0];
rz(1.5*pi) node[3];
cx node[14],node[0];
cx node[2],node[1];
cx node[4],node[3];
rz(3.9167583903239787*pi) node[0];
cx node[1],node[2];
cx node[3],node[4];
rz(0.5*pi) node[14];
cx node[2],node[1];
cx node[4],node[3];
sx node[14];
cx node[1],node[2];
rz(0.5*pi) node[14];
cx node[3],node[2];
sx node[14];
cx node[3],node[2];
rz(1.3885235418416768*pi) node[14];
cx node[14],node[0];
cx node[2],node[3];
cx node[0],node[14];
cx node[3],node[2];
cx node[14],node[0];
cx node[4],node[3];
cx node[1],node[0];
sx node[3];
cx node[1],node[0];
rz(2.5*pi) node[3];
cx node[0],node[1];
sx node[3];
cx node[1],node[0];
rz(1.5*pi) node[3];
cx node[0],node[14];
cx node[2],node[1];
cx node[4],node[3];
sx node[0];
cx node[1],node[2];
cx node[3],node[4];
rz(2.7437941719215124*pi) node[0];
cx node[2],node[1];
cx node[4],node[3];
sx node[0];
cx node[1],node[2];
rz(1.0*pi) node[0];
cx node[3],node[2];
cx node[14],node[0];
cx node[3],node[2];
cx node[0],node[14];
cx node[2],node[3];
cx node[14],node[0];
cx node[3],node[2];
cx node[1],node[0];
cx node[4],node[3];
rz(0.5*pi) node[1];
sx node[3];
sx node[1];
rz(2.5*pi) node[3];
rz(2.5*pi) node[1];
sx node[3];
sx node[1];
rz(1.5*pi) node[3];
rz(0.893859656449997*pi) node[1];
cx node[4],node[3];
cx node[1],node[0];
cx node[3],node[4];
cx node[0],node[1];
cx node[4],node[3];
cx node[1],node[0];
cx node[14],node[0];
cx node[2],node[1];
sx node[0];
rz(0.5*pi) node[2];
rz(2.5*pi) node[0];
sx node[2];
sx node[0];
rz(0.5*pi) node[2];
rz(1.5*pi) node[0];
sx node[2];
cx node[14],node[0];
rz(1.4835348527679382*pi) node[2];
cx node[0],node[14];
cx node[1],node[2];
cx node[14],node[0];
cx node[2],node[1];
cx node[1],node[2];
cx node[0],node[1];
cx node[3],node[2];
cx node[0],node[1];
rz(0.5*pi) node[3];
cx node[1],node[0];
sx node[3];
cx node[0],node[1];
rz(2.5*pi) node[3];
cx node[14],node[0];
sx node[3];
sx node[0];
rz(3.9527123094946974*pi) node[3];
rz(2.5*pi) node[0];
cx node[2],node[3];
sx node[0];
cx node[3],node[2];
rz(1.5*pi) node[0];
cx node[2],node[3];
cx node[14],node[0];
cx node[1],node[2];
cx node[4],node[3];
cx node[0],node[14];
cx node[1],node[2];
rz(1.549652152746778*pi) node[3];
rz(0.5*pi) node[4];
cx node[14],node[0];
cx node[2],node[1];
sx node[4];
cx node[1],node[2];
rz(0.5*pi) node[4];
cx node[0],node[1];
sx node[4];
cx node[0],node[1];
rz(0.6441587869622735*pi) node[4];
cx node[1],node[0];
cx node[4],node[3];
cx node[0],node[1];
cx node[3],node[4];
cx node[14],node[0];
cx node[4],node[3];
sx node[0];
cx node[2],node[3];
rz(2.5*pi) node[0];
cx node[2],node[3];
sx node[0];
cx node[3],node[2];
rz(1.5*pi) node[0];
cx node[2],node[3];
cx node[14],node[0];
cx node[1],node[2];
cx node[3],node[4];
cx node[0],node[14];
cx node[1],node[2];
sx node[3];
cx node[14],node[0];
cx node[2],node[1];
rz(3.445552479845536*pi) node[3];
cx node[1],node[2];
sx node[3];
cx node[0],node[1];
rz(1.0*pi) node[3];
cx node[14],node[0];
cx node[4],node[3];
cx node[0],node[1];
cx node[3],node[4];
cx node[14],node[0];
cx node[4],node[3];
cx node[0],node[1];
cx node[2],node[3];
sx node[1];
sx node[2];
rz(2.5*pi) node[1];
rz(3.1933272963152666*pi) node[2];
sx node[1];
sx node[2];
rz(1.5*pi) node[1];
rz(1.0*pi) node[2];
cx node[3],node[2];
cx node[2],node[3];
cx node[3],node[2];
cx node[2],node[1];
cx node[1],node[2];
cx node[2],node[1];
cx node[0],node[1];
sx node[0];
rz(2.0948727280311044*pi) node[0];
sx node[0];
rz(1.0*pi) node[0];
cx node[14],node[0];
cx node[0],node[14];
cx node[14],node[0];
cx node[0],node[1];
sx node[0];
cx node[2],node[1];
rz(2.721782909453393*pi) node[0];
rz(1.6017581216191306*pi) node[1];
sx node[2];
sx node[0];
sx node[1];
rz(2.0606476310334916*pi) node[2];
rz(1.0*pi) node[0];
rz(0.5*pi) node[1];
sx node[2];
sx node[1];
rz(1.0*pi) node[2];
rz(1.5*pi) node[1];
barrier node[4],node[3],node[14],node[0],node[2],node[1];
measure node[4] -> meas[0];
measure node[3] -> meas[1];
measure node[14] -> meas[2];
measure node[0] -> meas[3];
measure node[2] -> meas[4];
measure node[1] -> meas[5];
