OPENQASM 2.0;
include "qelib1.inc";

qreg node[15];
creg c[11];
sx node[0];
rz(2.032470698264092*pi) node[1];
sx node[2];
sx node[3];
sx node[4];
sx node[5];
rz(1.5*pi) node[6];
sx node[7];
rz(1.5*pi) node[10];
sx node[11];
rz(1.5*pi) node[12];
rz(3.5*pi) node[0];
rz(3.5*pi) node[2];
rz(3.5*pi) node[3];
rz(3.5*pi) node[4];
rz(3.5*pi) node[5];
sx node[6];
rz(3.5*pi) node[7];
sx node[10];
rz(0.5*pi) node[11];
sx node[12];
sx node[0];
sx node[2];
sx node[3];
sx node[4];
sx node[5];
rz(0.5*pi) node[6];
sx node[7];
rz(3.5*pi) node[10];
sx node[11];
rz(0.5*pi) node[12];
rz(1.0*pi) node[0];
rz(1.0*pi) node[2];
rz(1.0*pi) node[3];
rz(1.0*pi) node[4];
rz(1.0*pi) node[5];
sx node[6];
rz(1.0*pi) node[7];
sx node[10];
sx node[12];
cx node[1],node[0];
rz(0.5*pi) node[6];
rz(0.5*pi) node[10];
rz(0.5*pi) node[12];
rz(0.46752929763715967*pi) node[0];
x node[0];
rz(0.5*pi) node[0];
cx node[1],node[0];
rz(0.03295898361283989*pi) node[0];
cx node[1],node[2];
rz(0.4350585936827699*pi) node[2];
x node[2];
rz(0.5*pi) node[2];
cx node[1],node[2];
cx node[1],node[4];
rz(2.0659179688172298*pi) node[2];
cx node[3],node[2];
rz(0.3701171873655398*pi) node[4];
cx node[2],node[3];
x node[4];
cx node[3],node[2];
rz(0.5*pi) node[4];
cx node[1],node[4];
cx node[5],node[3];
cx node[1],node[2];
cx node[3],node[5];
rz(2.13183593763446*pi) node[4];
rz(0.24023436836488177*pi) node[2];
cx node[5],node[3];
cx node[7],node[4];
x node[2];
cx node[4],node[7];
cx node[8],node[5];
rz(0.5*pi) node[2];
cx node[7],node[4];
cx node[5],node[8];
cx node[1],node[2];
cx node[8],node[5];
cx node[10],node[7];
cx node[1],node[4];
rz(2.263671881635118*pi) node[2];
cx node[7],node[10];
cx node[11],node[8];
cx node[3],node[2];
rz(0.9804687515025912*pi) node[4];
cx node[10],node[7];
cx node[8],node[11];
cx node[2],node[3];
x node[4];
cx node[11],node[8];
cx node[12],node[10];
cx node[3],node[2];
rz(0.5*pi) node[4];
cx node[10],node[12];
cx node[1],node[4];
cx node[5],node[3];
cx node[12],node[10];
cx node[1],node[2];
cx node[3],node[5];
rz(1.5273437484974086*pi) node[4];
rz(0.4609374993732005*pi) node[2];
cx node[5],node[3];
cx node[7],node[4];
x node[2];
cx node[4],node[7];
cx node[8],node[5];
rz(0.5*pi) node[2];
cx node[7],node[4];
cx node[5],node[8];
cx node[1],node[2];
cx node[8],node[5];
cx node[6],node[7];
rz(0.5*pi) node[1];
rz(0.05468750062679906*pi) node[2];
cx node[7],node[6];
cx node[8],node[11];
sx node[1];
cx node[3],node[2];
cx node[6],node[7];
cx node[11],node[8];
rz(3.5*pi) node[1];
cx node[2],node[3];
cx node[8],node[11];
sx node[1];
cx node[3],node[2];
rz(1.0*pi) node[1];
cx node[5],node[3];
cx node[1],node[4];
cx node[3],node[5];
rz(1.5*pi) node[1];
cx node[5],node[3];
sx node[1];
rz(3.0781249996620494*pi) node[1];
sx node[1];
rz(1.5*pi) node[1];
cx node[1],node[4];
sx node[4];
rz(3.5*pi) node[4];
sx node[4];
rz(3.6093749996620494*pi) node[4];
cx node[7],node[4];
cx node[4],node[7];
cx node[7],node[4];
cx node[1],node[4];
cx node[10],node[7];
rz(1.5*pi) node[1];
cx node[7],node[10];
sx node[1];
cx node[10],node[7];
rz(0.15625000000000022*pi) node[1];
sx node[1];
rz(0.5*pi) node[1];
cx node[1],node[4];
sx node[1];
sx node[4];
cx node[1],node[2];
rz(3.5*pi) node[4];
rz(0.5*pi) node[1];
sx node[4];
sx node[1];
rz(3.718749999999999*pi) node[4];
rz(1.6875*pi) node[1];
cx node[7],node[4];
sx node[1];
cx node[4],node[7];
rz(2.5*pi) node[1];
cx node[7],node[4];
cx node[1],node[2];
sx node[1];
sx node[2];
cx node[1],node[4];
rz(2.5*pi) node[2];
rz(0.5*pi) node[1];
sx node[2];
sx node[1];
rz(1.4374999999999996*pi) node[2];
rz(1.6250000000000004*pi) node[1];
cx node[3],node[2];
sx node[1];
cx node[2],node[3];
rz(1.0*pi) node[1];
cx node[3],node[2];
cx node[1],node[4];
rz(0.5*pi) node[1];
sx node[4];
sx node[1];
rz(3.5*pi) node[4];
rz(3.5*pi) node[1];
sx node[4];
sx node[1];
rz(1.375*pi) node[4];
rz(1.0*pi) node[1];
cx node[1],node[2];
rz(3.75*pi) node[2];
cx node[1],node[2];
cx node[4],node[1];
rz(1.25*pi) node[2];
cx node[1],node[4];
sx node[2];
cx node[4],node[1];
rz(3.5*pi) node[2];
sx node[2];
cx node[7],node[4];
rz(1.0*pi) node[2];
cx node[4],node[7];
cx node[1],node[2];
cx node[7],node[4];
rz(0.25*pi) node[2];
cx node[10],node[7];
cx node[1],node[2];
cx node[7],node[10];
rz(0.5*pi) node[1];
rz(3.75*pi) node[2];
cx node[10],node[7];
sx node[1];
cx node[3],node[2];
rz(3.5*pi) node[1];
rz(0.125*pi) node[2];
sx node[1];
cx node[3],node[2];
rz(1.0*pi) node[1];
rz(3.875*pi) node[2];
cx node[1],node[2];
cx node[2],node[1];
cx node[1],node[2];
cx node[4],node[1];
cx node[3],node[2];
rz(0.0625*pi) node[1];
rz(0.25*pi) node[2];
cx node[4],node[1];
cx node[3],node[2];
rz(3.9375*pi) node[1];
rz(3.75*pi) node[2];
rz(0.5*pi) node[3];
cx node[1],node[4];
sx node[3];
cx node[4],node[1];
rz(3.5*pi) node[3];
cx node[1],node[4];
sx node[3];
cx node[1],node[2];
rz(1.0*pi) node[3];
cx node[7],node[4];
rz(0.125*pi) node[2];
rz(0.03125*pi) node[4];
cx node[1],node[2];
cx node[7],node[4];
rz(3.875*pi) node[2];
rz(3.96875*pi) node[4];
cx node[1],node[4];
cx node[4],node[1];
cx node[1],node[4];
cx node[1],node[2];
cx node[2],node[1];
cx node[1],node[2];
cx node[1],node[4];
cx node[3],node[2];
cx node[4],node[1];
cx node[2],node[3];
cx node[1],node[4];
cx node[3],node[2];
cx node[1],node[2];
cx node[5],node[3];
cx node[7],node[4];
rz(0.25*pi) node[2];
rz(0.015625*pi) node[3];
rz(0.0625*pi) node[4];
cx node[1],node[2];
cx node[5],node[3];
cx node[7],node[4];
rz(0.5*pi) node[1];
rz(3.75*pi) node[2];
rz(3.984375*pi) node[3];
rz(3.9375*pi) node[4];
sx node[1];
cx node[2],node[3];
rz(3.5*pi) node[1];
cx node[3],node[2];
sx node[1];
cx node[2],node[3];
rz(1.0*pi) node[1];
cx node[4],node[1];
cx node[1],node[4];
cx node[4],node[1];
cx node[1],node[2];
cx node[7],node[4];
cx node[2],node[1];
cx node[4],node[7];
cx node[1],node[2];
cx node[7],node[4];
cx node[1],node[4];
cx node[2],node[3];
cx node[4],node[1];
cx node[3],node[2];
cx node[1],node[4];
cx node[2],node[3];
cx node[1],node[2];
cx node[5],node[3];
cx node[7],node[4];
rz(0.125*pi) node[2];
rz(0.03125*pi) node[3];
cx node[4],node[7];
cx node[1],node[2];
cx node[5],node[3];
cx node[7],node[4];
cx node[1],node[4];
rz(3.875*pi) node[2];
rz(3.96875*pi) node[3];
cx node[6],node[7];
cx node[2],node[3];
rz(0.25*pi) node[4];
rz(0.0078125*pi) node[7];
cx node[1],node[4];
cx node[3],node[2];
cx node[6],node[7];
rz(0.5*pi) node[1];
cx node[2],node[3];
rz(3.75*pi) node[4];
rz(3.9921875*pi) node[7];
sx node[1];
cx node[5],node[3];
cx node[7],node[10];
rz(3.5*pi) node[1];
rz(0.0625*pi) node[3];
cx node[10],node[7];
sx node[1];
cx node[5],node[3];
cx node[7],node[10];
rz(1.0*pi) node[1];
rz(3.9375*pi) node[3];
cx node[6],node[7];
cx node[10],node[12];
cx node[4],node[1];
cx node[7],node[6];
cx node[12],node[10];
cx node[1],node[4];
cx node[6],node[7];
cx node[10],node[12];
cx node[4],node[1];
cx node[12],node[13];
cx node[1],node[2];
cx node[13],node[12];
cx node[2],node[1];
cx node[12],node[13];
cx node[1],node[2];
cx node[10],node[12];
cx node[13],node[14];
cx node[1],node[4];
cx node[2],node[3];
cx node[12],node[10];
cx node[14],node[13];
cx node[4],node[1];
cx node[3],node[2];
cx node[10],node[12];
cx node[13],node[14];
cx node[1],node[4];
cx node[2],node[3];
cx node[11],node[14];
cx node[12],node[13];
cx node[1],node[2];
cx node[5],node[3];
cx node[7],node[4];
cx node[13],node[12];
rz(0.00390625*pi) node[14];
cx node[2],node[1];
rz(0.125*pi) node[3];
rz(0.015625*pi) node[4];
cx node[11],node[14];
cx node[12],node[13];
cx node[1],node[2];
cx node[5],node[3];
cx node[7],node[4];
cx node[11],node[8];
rz(3.99609375*pi) node[14];
rz(3.875*pi) node[3];
rz(3.984375*pi) node[4];
cx node[8],node[11];
cx node[13],node[14];
cx node[1],node[4];
cx node[2],node[3];
cx node[11],node[8];
rz(0.001953125*pi) node[14];
cx node[4],node[1];
cx node[3],node[2];
cx node[13],node[14];
cx node[1],node[4];
cx node[2],node[3];
rz(3.998046875*pi) node[14];
cx node[2],node[1];
cx node[5],node[3];
cx node[7],node[4];
cx node[11],node[14];
cx node[1],node[2];
rz(0.25*pi) node[3];
rz(0.03125*pi) node[4];
rz(0.0009765625*pi) node[14];
cx node[2],node[1];
cx node[5],node[3];
cx node[7],node[4];
cx node[11],node[14];
rz(3.75*pi) node[3];
rz(3.96875*pi) node[4];
rz(0.5*pi) node[5];
rz(3.9990234375*pi) node[14];
cx node[3],node[2];
cx node[7],node[4];
sx node[5];
cx node[14],node[13];
cx node[2],node[3];
cx node[4],node[7];
rz(3.5*pi) node[5];
cx node[13],node[14];
cx node[3],node[2];
cx node[7],node[4];
sx node[5];
cx node[14],node[13];
cx node[4],node[1];
rz(1.0*pi) node[5];
rz(0.0625*pi) node[1];
cx node[5],node[3];
cx node[4],node[1];
cx node[3],node[5];
rz(3.9375*pi) node[1];
cx node[5],node[3];
cx node[4],node[1];
cx node[8],node[5];
cx node[1],node[4];
rz(0.0078125*pi) node[5];
cx node[4],node[1];
cx node[8],node[5];
cx node[1],node[2];
rz(3.9921875*pi) node[5];
rz(0.125*pi) node[2];
cx node[5],node[8];
cx node[1],node[2];
cx node[8],node[5];
rz(3.875*pi) node[2];
cx node[5],node[8];
cx node[3],node[2];
cx node[8],node[11];
cx node[2],node[3];
cx node[11],node[8];
cx node[3],node[2];
cx node[8],node[11];
cx node[1],node[2];
cx node[5],node[3];
cx node[14],node[11];
rz(0.25*pi) node[2];
cx node[3],node[5];
rz(0.00390625*pi) node[11];
cx node[1],node[2];
cx node[5],node[3];
cx node[14],node[11];
rz(0.5*pi) node[1];
rz(3.75*pi) node[2];
rz(3.99609375*pi) node[11];
sx node[1];
cx node[3],node[2];
cx node[8],node[11];
rz(3.5*pi) node[1];
cx node[2],node[3];
rz(0.001953125*pi) node[11];
sx node[1];
cx node[3],node[2];
cx node[8],node[11];
rz(1.0*pi) node[1];
rz(3.998046875*pi) node[11];
cx node[0],node[1];
cx node[1],node[0];
cx node[0],node[1];
cx node[1],node[4];
cx node[4],node[1];
cx node[1],node[4];
cx node[2],node[1];
cx node[7],node[4];
cx node[1],node[2];
cx node[4],node[7];
cx node[2],node[1];
cx node[7],node[4];
cx node[1],node[4];
cx node[7],node[10];
rz(0.015625*pi) node[4];
cx node[10],node[7];
cx node[1],node[4];
cx node[7],node[10];
cx node[1],node[2];
rz(3.984375*pi) node[4];
cx node[10],node[12];
rz(0.03125*pi) node[2];
cx node[12],node[10];
cx node[1],node[2];
cx node[10],node[12];
rz(3.96875*pi) node[2];
cx node[12],node[13];
cx node[1],node[2];
rz(0.00048828125*pi) node[13];
cx node[2],node[1];
cx node[12],node[13];
cx node[1],node[2];
rz(3.99951171875*pi) node[13];
cx node[12],node[13];
cx node[13],node[12];
cx node[12],node[13];
cx node[13],node[14];
cx node[14],node[13];
cx node[13],node[14];
cx node[14],node[11];
cx node[13],node[12];
rz(0.0009765625*pi) node[11];
cx node[12],node[13];
cx node[14],node[11];
cx node[13],node[12];
cx node[12],node[10];
rz(3.9990234375*pi) node[11];
cx node[10],node[12];
cx node[12],node[10];
cx node[10],node[7];
cx node[7],node[10];
cx node[10],node[7];
cx node[7],node[4];
rz(0.0078125*pi) node[4];
cx node[7],node[4];
rz(3.9921875*pi) node[4];
cx node[1],node[4];
cx node[4],node[1];
cx node[1],node[4];
cx node[1],node[2];
cx node[7],node[4];
cx node[2],node[1];
rz(0.015625*pi) node[4];
cx node[1],node[2];
cx node[7],node[4];
cx node[2],node[3];
rz(3.984375*pi) node[4];
cx node[3],node[2];
cx node[4],node[7];
cx node[2],node[3];
cx node[7],node[4];
cx node[1],node[2];
cx node[5],node[3];
cx node[4],node[7];
cx node[2],node[1];
cx node[3],node[5];
cx node[1],node[2];
cx node[5],node[3];
cx node[1],node[4];
cx node[8],node[5];
cx node[4],node[1];
rz(0.00390625*pi) node[5];
cx node[1],node[4];
cx node[8],node[5];
rz(3.99609375*pi) node[5];
cx node[8],node[5];
cx node[5],node[8];
cx node[8],node[5];
cx node[3],node[5];
cx node[8],node[11];
cx node[5],node[3];
cx node[11],node[8];
cx node[3],node[5];
cx node[8],node[11];
cx node[2],node[3];
cx node[14],node[11];
cx node[3],node[2];
rz(0.001953125*pi) node[11];
cx node[2],node[3];
cx node[14],node[11];
cx node[1],node[2];
cx node[3],node[5];
rz(3.998046875*pi) node[11];
cx node[2],node[1];
rz(0.0625*pi) node[5];
cx node[1],node[2];
cx node[3],node[5];
cx node[1],node[4];
cx node[2],node[3];
rz(3.9375*pi) node[5];
cx node[4],node[1];
cx node[3],node[2];
cx node[1],node[4];
cx node[2],node[3];
cx node[2],node[1];
cx node[3],node[5];
cx node[4],node[7];
rz(0.125*pi) node[1];
rz(0.03125*pi) node[5];
rz(0.0078125*pi) node[7];
cx node[2],node[1];
cx node[3],node[5];
cx node[4],node[7];
rz(3.875*pi) node[1];
rz(3.96875*pi) node[5];
rz(3.9921875*pi) node[7];
cx node[1],node[4];
cx node[7],node[10];
cx node[4],node[1];
cx node[10],node[7];
cx node[1],node[4];
cx node[7],node[10];
cx node[2],node[1];
cx node[10],node[12];
cx node[1],node[2];
cx node[12],node[10];
cx node[2],node[1];
cx node[10],node[12];
cx node[1],node[0];
cx node[2],node[3];
cx node[12],node[13];
rz(0.25*pi) node[0];
cx node[3],node[2];
cx node[13],node[12];
cx node[1],node[0];
cx node[2],node[3];
cx node[12],node[13];
rz(3.75*pi) node[0];
rz(0.5*pi) node[1];
cx node[3],node[5];
cx node[14],node[13];
sx node[1];
rz(0.015625*pi) node[5];
rz(0.00390625*pi) node[13];
rz(3.5*pi) node[1];
cx node[3],node[5];
cx node[14],node[13];
sx node[1];
rz(3.984375*pi) node[5];
cx node[14],node[11];
rz(3.99609375*pi) node[13];
rz(1.0*pi) node[1];
cx node[11],node[14];
cx node[4],node[1];
cx node[14],node[11];
cx node[1],node[4];
cx node[11],node[8];
cx node[4],node[1];
cx node[8],node[11];
cx node[2],node[1];
cx node[11],node[8];
rz(0.0625*pi) node[1];
cx node[8],node[5];
cx node[2],node[1];
rz(0.0078125*pi) node[5];
rz(3.9375*pi) node[1];
cx node[8],node[5];
cx node[2],node[1];
rz(3.9921875*pi) node[5];
cx node[1],node[2];
cx node[8],node[5];
cx node[2],node[1];
cx node[5],node[8];
cx node[1],node[0];
cx node[3],node[2];
cx node[8],node[5];
rz(0.125*pi) node[0];
rz(0.03125*pi) node[2];
cx node[1],node[0];
cx node[3],node[2];
rz(3.875*pi) node[0];
cx node[1],node[4];
rz(3.96875*pi) node[2];
cx node[3],node[2];
rz(0.25*pi) node[4];
cx node[1],node[4];
cx node[2],node[3];
rz(0.5*pi) node[1];
cx node[3],node[2];
rz(3.75*pi) node[4];
sx node[1];
cx node[5],node[3];
rz(3.5*pi) node[1];
rz(0.015625*pi) node[3];
sx node[1];
cx node[5],node[3];
rz(1.0*pi) node[1];
rz(3.984375*pi) node[3];
cx node[2],node[1];
cx node[5],node[3];
cx node[1],node[2];
cx node[3],node[5];
cx node[2],node[1];
cx node[5],node[3];
cx node[1],node[0];
rz(0.0625*pi) node[0];
cx node[1],node[0];
rz(3.9375*pi) node[0];
cx node[1],node[4];
rz(0.125*pi) node[4];
cx node[1],node[4];
cx node[1],node[2];
rz(3.875*pi) node[4];
rz(0.25*pi) node[2];
cx node[1],node[2];
rz(0.5*pi) node[1];
rz(3.75*pi) node[2];
sx node[1];
cx node[3],node[2];
rz(3.5*pi) node[1];
cx node[2],node[3];
sx node[1];
cx node[3],node[2];
rz(1.0*pi) node[1];
cx node[2],node[1];
cx node[1],node[2];
cx node[2],node[1];
cx node[1],node[0];
rz(0.03125*pi) node[0];
cx node[1],node[0];
rz(3.96875*pi) node[0];
cx node[1],node[4];
rz(0.0625*pi) node[4];
cx node[1],node[4];
cx node[1],node[2];
rz(3.9375*pi) node[4];
cx node[2],node[1];
cx node[1],node[2];
cx node[2],node[3];
rz(0.125*pi) node[3];
cx node[2],node[3];
cx node[2],node[1];
rz(3.875*pi) node[3];
rz(0.25*pi) node[1];
cx node[2],node[1];
rz(3.75*pi) node[1];
rz(0.5*pi) node[2];
sx node[2];
rz(3.5*pi) node[2];
sx node[2];
rz(1.0*pi) node[2];
barrier node[12],node[11],node[14],node[13],node[8],node[5],node[0],node[4],node[3],node[1],node[2],node[6];
measure node[12] -> c[0];
measure node[11] -> c[1];
measure node[14] -> c[2];
measure node[13] -> c[3];
measure node[8] -> c[4];
measure node[5] -> c[5];
measure node[0] -> c[6];
measure node[4] -> c[7];
measure node[3] -> c[8];
measure node[1] -> c[9];
measure node[2] -> c[10];
