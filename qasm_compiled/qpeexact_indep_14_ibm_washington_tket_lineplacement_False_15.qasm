OPENQASM 2.0;
include "qelib1.inc";

qreg node[23];
creg c[13];
rz(2.5712890647268734*pi) node[0];
sx node[1];
sx node[2];
rz(0.5*pi) node[3];
rz(3.5*pi) node[4];
sx node[5];
rz(1.5*pi) node[6];
sx node[14];
sx node[15];
sx node[19];
sx node[20];
x node[21];
x node[22];
x node[0];
rz(0.5*pi) node[1];
rz(2.5*pi) node[2];
sx node[3];
x node[4];
rz(0.5*pi) node[5];
sx node[6];
rz(0.5*pi) node[14];
rz(0.5*pi) node[15];
rz(0.5*pi) node[19];
rz(0.5*pi) node[20];
sx node[21];
sx node[22];
rz(0.5*pi) node[0];
sx node[1];
sx node[2];
rz(3.5*pi) node[3];
rz(0.5*pi) node[4];
sx node[5];
rz(0.5*pi) node[6];
sx node[14];
sx node[15];
sx node[19];
sx node[20];
cx node[0],node[1];
rz(2.1249999999999996*pi) node[2];
sx node[3];
sx node[6];
rz(0.4287109433048286*pi) node[1];
rz(1.0*pi) node[3];
rz(0.5*pi) node[6];
cx node[0],node[1];
cx node[4],node[3];
cx node[0],node[14];
rz(3.571411127007671*pi) node[1];
rz(0.5*pi) node[4];
sx node[4];
rz(3.8574218750199285*pi) node[14];
cx node[0],node[14];
rz(2.75*pi) node[4];
rz(0.5*pi) node[0];
sx node[4];
rz(2.142822265605071*pi) node[14];
sx node[0];
rz(1.0*pi) node[4];
cx node[18],node[14];
rz(3.5*pi) node[0];
cx node[4],node[3];
cx node[14],node[18];
sx node[0];
sx node[3];
sx node[4];
cx node[18],node[14];
rz(1.0*pi) node[0];
rz(2.5*pi) node[3];
rz(2.25*pi) node[4];
cx node[19],node[18];
cx node[14],node[0];
sx node[3];
sx node[4];
cx node[18],node[19];
rz(2.25*pi) node[3];
rz(3.5*pi) node[4];
rz(0.5*pi) node[14];
cx node[19],node[18];
cx node[2],node[3];
cx node[5],node[4];
sx node[14];
cx node[20],node[19];
rz(0.125*pi) node[3];
cx node[4],node[5];
rz(2.7148437436736597*pi) node[14];
cx node[19],node[20];
cx node[2],node[3];
cx node[5],node[4];
sx node[14];
cx node[20],node[19];
rz(3.875*pi) node[3];
rz(1.0*pi) node[14];
cx node[14],node[0];
cx node[4],node[3];
sx node[0];
cx node[3],node[4];
rz(0.5*pi) node[14];
rz(0.5*pi) node[0];
cx node[4],node[3];
sx node[14];
sx node[0];
cx node[3],node[2];
cx node[15],node[4];
rz(3.5*pi) node[14];
cx node[2],node[3];
cx node[4],node[15];
sx node[14];
cx node[3],node[2];
cx node[15],node[4];
rz(1.2856445375763403*pi) node[14];
cx node[4],node[3];
cx node[18],node[14];
cx node[22],node[15];
cx node[3],node[4];
cx node[14],node[18];
cx node[15],node[22];
cx node[4],node[3];
cx node[18],node[14];
cx node[22],node[15];
cx node[0],node[14];
cx node[4],node[5];
cx node[19],node[18];
rz(0.25*pi) node[5];
rz(0.42968750212014617*pi) node[14];
cx node[18],node[19];
cx node[0],node[14];
cx node[4],node[5];
cx node[19],node[18];
rz(0.5*pi) node[4];
rz(3.75*pi) node[5];
rz(1.5712890603798533*pi) node[14];
sx node[4];
cx node[6],node[5];
cx node[18],node[14];
rz(3.5*pi) node[4];
cx node[5],node[6];
cx node[14],node[18];
sx node[4];
cx node[6],node[5];
cx node[18],node[14];
cx node[0],node[14];
rz(1.0*pi) node[4];
cx node[15],node[4];
rz(3.8593750006083107*pi) node[14];
cx node[0],node[14];
cx node[4],node[15];
cx node[0],node[1];
cx node[15],node[4];
rz(2.142578124391689*pi) node[14];
cx node[1],node[0];
cx node[0],node[1];
cx node[1],node[2];
rz(3.71875*pi) node[2];
cx node[1],node[2];
rz(2.2851562499999996*pi) node[2];
cx node[3],node[2];
cx node[2],node[3];
cx node[3],node[2];
cx node[1],node[2];
cx node[4],node[3];
rz(0.4375000000000002*pi) node[2];
cx node[3],node[4];
cx node[1],node[2];
cx node[4],node[3];
rz(0.5*pi) node[1];
rz(3.570312499999999*pi) node[2];
cx node[5],node[4];
sx node[1];
cx node[3],node[2];
cx node[4],node[5];
rz(3.5*pi) node[1];
cx node[2],node[3];
cx node[5],node[4];
sx node[1];
cx node[3],node[2];
cx node[6],node[5];
rz(1.0*pi) node[1];
cx node[4],node[3];
cx node[5],node[6];
cx node[1],node[2];
cx node[3],node[4];
cx node[6],node[5];
rz(0.5*pi) node[1];
cx node[4],node[3];
sx node[1];
cx node[5],node[4];
rz(0.8749999999999998*pi) node[1];
cx node[4],node[5];
sx node[1];
cx node[5],node[4];
rz(1.5*pi) node[1];
cx node[1],node[2];
sx node[2];
rz(2.5*pi) node[2];
sx node[2];
rz(0.6406249999999991*pi) node[2];
cx node[3],node[2];
cx node[2],node[3];
cx node[3],node[2];
cx node[1],node[2];
rz(1.5*pi) node[1];
sx node[1];
rz(0.25*pi) node[1];
sx node[1];
rz(1.0*pi) node[1];
cx node[1],node[2];
rz(0.5*pi) node[1];
sx node[2];
sx node[1];
rz(3.5*pi) node[2];
rz(3.5*pi) node[1];
sx node[2];
sx node[1];
rz(3.781249999999999*pi) node[2];
rz(1.0*pi) node[1];
cx node[2],node[3];
cx node[1],node[0];
cx node[3],node[2];
cx node[0],node[1];
cx node[2],node[3];
cx node[1],node[0];
cx node[0],node[14];
cx node[14],node[0];
cx node[0],node[14];
cx node[14],node[18];
cx node[18],node[14];
cx node[14],node[18];
cx node[18],node[19];
cx node[19],node[18];
cx node[18],node[19];
cx node[19],node[20];
cx node[20],node[21];
cx node[19],node[20];
cx node[20],node[21];
cx node[20],node[19];
sx node[21];
cx node[19],node[20];
rz(2.5*pi) node[21];
cx node[20],node[19];
sx node[21];
rz(0.5624999999999993*pi) node[21];
cx node[21],node[22];
rz(0.0625*pi) node[22];
cx node[21],node[22];
rz(3.9375*pi) node[22];
cx node[22],node[15];
cx node[15],node[22];
cx node[22],node[15];
cx node[15],node[4];
cx node[21],node[22];
cx node[4],node[15];
cx node[22],node[21];
cx node[15],node[4];
cx node[21],node[22];
cx node[3],node[4];
cx node[22],node[15];
rz(0.03125*pi) node[4];
rz(0.125*pi) node[15];
cx node[3],node[4];
cx node[22],node[15];
rz(3.96875*pi) node[4];
rz(3.875*pi) node[15];
cx node[22],node[21];
cx node[4],node[3];
rz(0.25*pi) node[21];
cx node[3],node[4];
cx node[22],node[21];
cx node[4],node[3];
rz(3.75*pi) node[21];
rz(0.5*pi) node[22];
cx node[2],node[3];
sx node[22];
rz(0.015625*pi) node[3];
rz(3.5*pi) node[22];
cx node[2],node[3];
sx node[22];
rz(3.984375*pi) node[3];
rz(1.0*pi) node[22];
cx node[3],node[4];
cx node[21],node[22];
cx node[4],node[3];
cx node[22],node[21];
cx node[3],node[4];
cx node[21],node[22];
cx node[5],node[4];
rz(0.0078125*pi) node[4];
cx node[5],node[4];
rz(3.9921875*pi) node[4];
cx node[6],node[5];
cx node[5],node[6];
cx node[6],node[5];
cx node[5],node[4];
rz(0.00390625*pi) node[4];
cx node[5],node[4];
rz(3.99609375*pi) node[4];
cx node[6],node[5];
cx node[4],node[3];
cx node[5],node[6];
cx node[3],node[4];
cx node[6],node[5];
cx node[4],node[3];
cx node[3],node[2];
cx node[4],node[15];
cx node[2],node[3];
rz(0.0625*pi) node[15];
cx node[3],node[2];
cx node[4],node[15];
cx node[2],node[1];
rz(3.9375*pi) node[15];
cx node[1],node[2];
cx node[15],node[4];
cx node[2],node[1];
cx node[4],node[15];
cx node[0],node[1];
cx node[15],node[4];
rz(0.001953125*pi) node[1];
cx node[3],node[4];
cx node[15],node[22];
cx node[0],node[1];
rz(0.03125*pi) node[4];
rz(0.125*pi) node[22];
rz(3.998046875*pi) node[1];
cx node[3],node[4];
cx node[15],node[22];
cx node[1],node[0];
rz(3.96875*pi) node[4];
rz(3.875*pi) node[22];
cx node[0],node[1];
cx node[5],node[4];
cx node[22],node[15];
cx node[1],node[0];
rz(0.015625*pi) node[4];
cx node[15],node[22];
cx node[14],node[0];
cx node[2],node[1];
cx node[5],node[4];
cx node[22],node[15];
rz(0.0009765625*pi) node[0];
cx node[1],node[2];
rz(3.984375*pi) node[4];
cx node[22],node[21];
cx node[14],node[0];
cx node[2],node[1];
cx node[4],node[5];
rz(0.25*pi) node[21];
rz(3.9990234375*pi) node[0];
cx node[5],node[4];
cx node[22],node[21];
cx node[0],node[14];
cx node[4],node[5];
rz(3.75*pi) node[21];
rz(0.5*pi) node[22];
cx node[14],node[0];
cx node[6],node[5];
sx node[22];
cx node[0],node[14];
rz(0.0078125*pi) node[5];
rz(3.5*pi) node[22];
cx node[1],node[0];
cx node[6],node[5];
cx node[18],node[14];
sx node[22];
cx node[0],node[1];
rz(3.9921875*pi) node[5];
rz(0.00048828125*pi) node[14];
rz(1.0*pi) node[22];
cx node[1],node[0];
cx node[5],node[4];
cx node[18],node[14];
cx node[21],node[22];
cx node[4],node[5];
rz(3.99951171875*pi) node[14];
cx node[19],node[18];
cx node[22],node[21];
cx node[5],node[4];
cx node[18],node[19];
cx node[21],node[22];
cx node[4],node[3];
cx node[19],node[18];
cx node[3],node[4];
cx node[18],node[14];
cx node[4],node[3];
rz(0.000244140625*pi) node[14];
cx node[2],node[3];
cx node[4],node[15];
cx node[18],node[14];
rz(0.00390625*pi) node[3];
rz(3.999755859375*pi) node[14];
rz(0.0625*pi) node[15];
cx node[19],node[18];
cx node[0],node[14];
cx node[2],node[3];
cx node[4],node[15];
cx node[18],node[19];
rz(3.99609375*pi) node[3];
rz(0.0001220703125*pi) node[14];
rz(3.9375*pi) node[15];
cx node[19],node[18];
cx node[0],node[14];
cx node[3],node[2];
cx node[4],node[15];
cx node[2],node[3];
cx node[15],node[4];
rz(3.9998779296875*pi) node[14];
cx node[3],node[2];
cx node[4],node[15];
cx node[1],node[2];
cx node[5],node[4];
cx node[15],node[22];
rz(0.001953125*pi) node[2];
rz(0.03125*pi) node[4];
rz(0.125*pi) node[22];
cx node[1],node[2];
cx node[5],node[4];
cx node[15],node[22];
rz(3.998046875*pi) node[2];
rz(3.96875*pi) node[4];
rz(3.875*pi) node[22];
cx node[2],node[1];
cx node[4],node[5];
cx node[22],node[15];
cx node[1],node[2];
cx node[5],node[4];
cx node[15],node[22];
cx node[2],node[1];
cx node[4],node[5];
cx node[22],node[15];
cx node[1],node[0];
cx node[4],node[15];
cx node[6],node[5];
cx node[22],node[21];
cx node[0],node[1];
rz(0.015625*pi) node[5];
rz(0.0625*pi) node[15];
rz(0.25*pi) node[21];
cx node[1],node[0];
cx node[4],node[15];
cx node[6],node[5];
cx node[22],node[21];
cx node[0],node[14];
rz(3.984375*pi) node[5];
rz(3.9375*pi) node[15];
rz(3.75*pi) node[21];
rz(0.5*pi) node[22];
cx node[14],node[0];
cx node[4],node[15];
sx node[22];
cx node[0],node[14];
cx node[15],node[4];
rz(3.5*pi) node[22];
cx node[4],node[15];
cx node[18],node[14];
sx node[22];
cx node[5],node[4];
rz(0.0009765625*pi) node[14];
rz(1.0*pi) node[22];
cx node[4],node[5];
cx node[18],node[14];
cx node[21],node[22];
cx node[5],node[4];
rz(3.9990234375*pi) node[14];
cx node[22],node[21];
cx node[3],node[4];
cx node[6],node[5];
cx node[18],node[14];
cx node[21],node[22];
rz(0.0078125*pi) node[4];
rz(0.03125*pi) node[5];
cx node[14],node[18];
cx node[15],node[22];
cx node[3],node[4];
cx node[6],node[5];
cx node[18],node[14];
rz(0.125*pi) node[22];
cx node[14],node[0];
rz(3.9921875*pi) node[4];
rz(3.96875*pi) node[5];
cx node[15],node[22];
cx node[19],node[18];
cx node[0],node[14];
cx node[4],node[3];
rz(0.00048828125*pi) node[18];
rz(3.875*pi) node[22];
cx node[14],node[0];
cx node[3],node[4];
cx node[22],node[15];
cx node[19],node[18];
cx node[0],node[1];
cx node[4],node[3];
cx node[15],node[22];
rz(3.99951171875*pi) node[18];
cx node[1],node[0];
cx node[2],node[3];
cx node[4],node[5];
cx node[18],node[14];
cx node[22],node[15];
cx node[0],node[1];
rz(0.00390625*pi) node[3];
rz(0.015625*pi) node[5];
cx node[14],node[18];
cx node[22],node[21];
cx node[2],node[3];
cx node[4],node[5];
cx node[18],node[14];
rz(0.25*pi) node[21];
rz(3.99609375*pi) node[3];
cx node[15],node[4];
rz(3.984375*pi) node[5];
cx node[19],node[18];
cx node[22],node[21];
cx node[2],node[3];
cx node[4],node[15];
cx node[18],node[19];
rz(3.75*pi) node[21];
rz(0.5*pi) node[22];
cx node[3],node[2];
cx node[15],node[4];
cx node[19],node[18];
sx node[22];
cx node[2],node[3];
cx node[5],node[4];
rz(3.5*pi) node[22];
cx node[1],node[2];
cx node[4],node[5];
sx node[22];
rz(0.001953125*pi) node[2];
cx node[5],node[4];
rz(1.0*pi) node[22];
cx node[1],node[2];
cx node[3],node[4];
cx node[6],node[5];
cx node[21],node[22];
rz(3.998046875*pi) node[2];
rz(0.0078125*pi) node[4];
rz(0.0625*pi) node[5];
cx node[22],node[21];
cx node[1],node[2];
cx node[3],node[4];
cx node[6],node[5];
cx node[21],node[22];
cx node[2],node[1];
rz(3.9921875*pi) node[4];
rz(3.9375*pi) node[5];
cx node[1],node[2];
cx node[5],node[6];
cx node[0],node[1];
cx node[6],node[5];
cx node[1],node[0];
cx node[5],node[6];
cx node[0],node[1];
cx node[5],node[4];
cx node[0],node[14];
cx node[4],node[5];
cx node[14],node[0];
cx node[5],node[4];
cx node[0],node[14];
cx node[4],node[15];
cx node[1],node[0];
cx node[15],node[4];
cx node[18],node[14];
rz(0.000244140625*pi) node[0];
cx node[4],node[15];
rz(0.0009765625*pi) node[14];
cx node[1],node[0];
cx node[5],node[4];
cx node[18],node[14];
cx node[15],node[22];
rz(3.999755859375*pi) node[0];
cx node[4],node[5];
rz(3.9990234375*pi) node[14];
rz(0.125*pi) node[22];
cx node[5],node[4];
cx node[14],node[18];
cx node[15],node[22];
cx node[4],node[3];
cx node[5],node[6];
cx node[18],node[14];
rz(3.875*pi) node[22];
cx node[3],node[4];
rz(0.03125*pi) node[6];
cx node[14],node[18];
cx node[22],node[15];
cx node[14],node[0];
cx node[4],node[3];
cx node[5],node[6];
cx node[15],node[22];
cx node[0],node[14];
cx node[2],node[3];
cx node[4],node[5];
rz(3.96875*pi) node[6];
cx node[22],node[15];
cx node[14],node[0];
rz(0.00390625*pi) node[3];
cx node[5],node[4];
cx node[22],node[21];
cx node[0],node[1];
cx node[2],node[3];
cx node[4],node[5];
cx node[18],node[14];
rz(0.25*pi) node[21];
cx node[1],node[0];
rz(3.99609375*pi) node[3];
cx node[4],node[15];
cx node[5],node[6];
cx node[14],node[18];
cx node[22],node[21];
cx node[0],node[1];
cx node[2],node[3];
rz(0.015625*pi) node[6];
cx node[18],node[14];
rz(0.0625*pi) node[15];
rz(3.75*pi) node[21];
rz(0.5*pi) node[22];
cx node[0],node[14];
cx node[3],node[2];
cx node[4],node[15];
cx node[5],node[6];
sx node[22];
cx node[2],node[3];
rz(3.984375*pi) node[6];
rz(0.00048828125*pi) node[14];
rz(3.9375*pi) node[15];
rz(3.5*pi) node[22];
cx node[0],node[14];
cx node[1],node[2];
cx node[4],node[15];
sx node[22];
rz(0.001953125*pi) node[2];
cx node[15],node[4];
rz(3.99951171875*pi) node[14];
rz(1.0*pi) node[22];
cx node[1],node[2];
cx node[4],node[15];
cx node[21],node[22];
rz(3.998046875*pi) node[2];
cx node[5],node[4];
cx node[22],node[21];
cx node[1],node[2];
rz(0.03125*pi) node[4];
cx node[21],node[22];
cx node[2],node[1];
cx node[5],node[4];
cx node[15],node[22];
cx node[1],node[2];
rz(3.96875*pi) node[4];
rz(0.125*pi) node[22];
cx node[0],node[1];
cx node[3],node[4];
cx node[15],node[22];
rz(0.0009765625*pi) node[1];
cx node[4],node[3];
rz(3.875*pi) node[22];
cx node[0],node[1];
cx node[3],node[4];
cx node[22],node[15];
rz(3.9990234375*pi) node[1];
cx node[2],node[3];
cx node[4],node[5];
cx node[15],node[22];
cx node[0],node[1];
cx node[3],node[2];
cx node[5],node[4];
cx node[22],node[15];
cx node[1],node[0];
cx node[2],node[3];
cx node[4],node[5];
cx node[22],node[21];
cx node[0],node[1];
cx node[4],node[15];
cx node[5],node[6];
rz(0.25*pi) node[21];
rz(0.0078125*pi) node[6];
rz(0.0625*pi) node[15];
cx node[22],node[21];
cx node[4],node[15];
cx node[5],node[6];
rz(3.75*pi) node[21];
rz(0.5*pi) node[22];
rz(3.9921875*pi) node[6];
rz(3.9375*pi) node[15];
sx node[22];
cx node[4],node[15];
rz(3.5*pi) node[22];
cx node[15],node[4];
sx node[22];
cx node[4],node[15];
rz(1.0*pi) node[22];
cx node[5],node[4];
cx node[21],node[22];
cx node[4],node[5];
cx node[22],node[21];
cx node[5],node[4];
cx node[21],node[22];
cx node[4],node[3];
cx node[15],node[22];
cx node[3],node[4];
rz(0.125*pi) node[22];
cx node[4],node[3];
cx node[15],node[22];
cx node[3],node[2];
cx node[5],node[4];
rz(3.875*pi) node[22];
rz(0.015625*pi) node[2];
cx node[4],node[5];
cx node[22],node[15];
cx node[3],node[2];
cx node[5],node[4];
cx node[15],node[22];
rz(3.984375*pi) node[2];
cx node[3],node[4];
cx node[5],node[6];
cx node[22],node[15];
rz(0.03125*pi) node[4];
rz(0.00390625*pi) node[6];
cx node[22],node[21];
cx node[3],node[4];
cx node[5],node[6];
rz(0.25*pi) node[21];
rz(3.96875*pi) node[4];
rz(3.99609375*pi) node[6];
cx node[22],node[21];
cx node[5],node[4];
rz(3.75*pi) node[21];
rz(0.5*pi) node[22];
cx node[4],node[5];
sx node[22];
cx node[5],node[4];
rz(3.5*pi) node[22];
cx node[4],node[3];
sx node[22];
cx node[3],node[4];
rz(1.0*pi) node[22];
cx node[4],node[3];
cx node[21],node[22];
cx node[3],node[2];
cx node[4],node[15];
cx node[22],node[21];
rz(0.0078125*pi) node[2];
rz(0.0625*pi) node[15];
cx node[21],node[22];
cx node[3],node[2];
cx node[4],node[15];
rz(3.9921875*pi) node[2];
rz(3.9375*pi) node[15];
cx node[1],node[2];
cx node[4],node[15];
cx node[2],node[1];
cx node[15],node[4];
cx node[1],node[2];
cx node[4],node[15];
cx node[3],node[4];
cx node[15],node[22];
cx node[4],node[3];
rz(0.125*pi) node[22];
cx node[3],node[4];
cx node[15],node[22];
cx node[4],node[5];
rz(3.875*pi) node[22];
rz(0.015625*pi) node[5];
cx node[22],node[15];
cx node[4],node[5];
cx node[15],node[22];
cx node[4],node[3];
rz(3.984375*pi) node[5];
cx node[22],node[15];
rz(0.03125*pi) node[3];
cx node[6],node[5];
cx node[22],node[21];
cx node[4],node[3];
cx node[5],node[6];
rz(0.25*pi) node[21];
rz(3.96875*pi) node[3];
cx node[4],node[15];
cx node[6],node[5];
cx node[22],node[21];
rz(0.0625*pi) node[15];
rz(3.75*pi) node[21];
rz(0.5*pi) node[22];
cx node[4],node[15];
sx node[22];
rz(3.9375*pi) node[15];
rz(3.5*pi) node[22];
cx node[4],node[15];
sx node[22];
cx node[15],node[4];
rz(1.0*pi) node[22];
cx node[4],node[15];
cx node[5],node[4];
cx node[15],node[22];
cx node[4],node[5];
cx node[22],node[15];
cx node[5],node[4];
cx node[15],node[22];
cx node[4],node[3];
cx node[6],node[5];
cx node[22],node[21];
cx node[3],node[4];
cx node[5],node[6];
rz(0.125*pi) node[21];
cx node[4],node[3];
cx node[6],node[5];
cx node[22],node[21];
cx node[2],node[3];
cx node[22],node[15];
rz(3.875*pi) node[21];
rz(0.001953125*pi) node[3];
rz(0.25*pi) node[15];
cx node[2],node[3];
cx node[22],node[15];
cx node[2],node[1];
rz(3.998046875*pi) node[3];
rz(3.75*pi) node[15];
rz(0.5*pi) node[22];
rz(0.00390625*pi) node[1];
sx node[22];
cx node[2],node[1];
rz(3.5*pi) node[22];
rz(3.99609375*pi) node[1];
cx node[2],node[3];
sx node[22];
cx node[3],node[2];
rz(1.0*pi) node[22];
cx node[2],node[3];
cx node[21],node[22];
cx node[3],node[4];
cx node[22],node[21];
cx node[4],node[3];
cx node[21],node[22];
cx node[3],node[4];
cx node[4],node[5];
rz(0.0078125*pi) node[5];
cx node[4],node[5];
cx node[4],node[3];
rz(3.9921875*pi) node[5];
rz(0.015625*pi) node[3];
cx node[6],node[5];
cx node[4],node[3];
cx node[5],node[6];
rz(3.984375*pi) node[3];
cx node[6],node[5];
cx node[4],node[5];
rz(0.03125*pi) node[5];
cx node[4],node[5];
cx node[4],node[15];
rz(3.96875*pi) node[5];
cx node[15],node[4];
cx node[4],node[15];
cx node[15],node[22];
rz(0.0625*pi) node[22];
cx node[15],node[22];
cx node[15],node[4];
rz(3.9375*pi) node[22];
rz(0.125*pi) node[4];
cx node[21],node[22];
cx node[15],node[4];
cx node[22],node[21];
rz(3.875*pi) node[4];
cx node[21],node[22];
cx node[15],node[22];
rz(0.25*pi) node[22];
cx node[15],node[22];
rz(0.5*pi) node[15];
rz(3.75*pi) node[22];
sx node[15];
rz(3.5*pi) node[15];
sx node[15];
rz(1.0*pi) node[15];
barrier node[19],node[18],node[14],node[0],node[2],node[1],node[6],node[3],node[5],node[21],node[4],node[22],node[15],node[20];
measure node[19] -> c[0];
measure node[18] -> c[1];
measure node[14] -> c[2];
measure node[0] -> c[3];
measure node[2] -> c[4];
measure node[1] -> c[5];
measure node[6] -> c[6];
measure node[3] -> c[7];
measure node[5] -> c[8];
measure node[21] -> c[9];
measure node[4] -> c[10];
measure node[22] -> c[11];
measure node[15] -> c[12];
