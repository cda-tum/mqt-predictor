...
def read_baseline_date(path: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    worst_overall_values = np.maximum.reduce([baseline for baseline in sorted_data])
    best_overall_values = np.minimum.reduce([baseline for baseline in sorted_data])
    return best_run, worst_run, median_run, best_overall_values, worst_overall_values,
...
def generate_eval_plot(
    ...
    plt.fill_between(range(len(best_overall_O1)), best_overall_O1, worst_overall_O1, alpha=0.2, color=colors[0], label="Qiskit Default")
    plt.fill_between(range(len(best_overall_O3)), best_overall_O3, worst_overall_O3, alpha=0.5, color=colors[2], label="Qiskit Most-optimized")
