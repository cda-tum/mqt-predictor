...
def test_train_and_predictor_random_forest_classifier() -> None:
    ...
    prediction = predictor.predict_probs(qc)
    ...
    prediction = predictor.predict_probs(
        file,
    )
    for elem in prediction:
        assert 0 <= elem <= 1
    ...
    with pytest.raises(
        ...
        ml.helper.predict_device_for_figure_of_merit(qc, figure_of_merit="false_input")  # type: ignore[arg-type]
...
def test_performance_measures() -> None:
    ...
    training_data = predictor.get_prepared_training_data(save_non_zero_indices=False)
